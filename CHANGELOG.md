...
* patched bson-error.c for strerror_l error on Andriod build
* in cli_args.cpp, changed std::strtol to ::strtol due to NDK 29 error
* in micros.h changed PROPAGATE_POD_FROM_TEMPLATE_ARGUMENT and DECLARE_POD due to NDK 29 error
* in scope_exit.hpp, changed std::is_nothrow_destructible due to NDK 29 error
...
* Properties in the frozen *before* Realm instance in the client reset callbacks may have had properties reordered which could lead to exceptions if accessed. ([PR 6693](https://github.com/realm/realm-core/issues/6693), since v13.11.0)
...
* Switch back to default off_t size so consumers don't need to define_FILE_OFFSET_BITS=64
...
* Changed `Group::remove_table`, `Group::TransactAdvancer::insert_group_level_table`
  and `Group::TransactAdvancer::erase_group_level_table` from *move-last-over* to
...
* New macro REALM_DEBUG to control compilation mode.
    The library (including all headers) is no longer affected in any way by the definition status of NDEBUG or_DEBUG.
